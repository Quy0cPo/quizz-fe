      ...
      {players.map((player, index) => {
        ...
        return (
          <motion.li 
            ...
            className={cn(
              "flex items-center justify-between p-2.5 sm:p-3 rounded-xl border transition-all bg-white",
              submission ? (isCorrect ? "bg-emerald-50 border-emerald-200 shadow-[0_0_15px_rgba(16,185,129,0.2)]" : "bg-red-50 border-red-200 opacity-80") : "border-slate-100 shadow-sm"
            )}
          >
            <div className="flex items-center gap-2 sm:gap-3 overflow-hidden">
              <span className="flex-shrink-0 w-7 h-7 flex items-center justify-center bg-slate-100 text-slate-500 font-bold rounded-full text-xs">
                {index + 1}
              </span>
              
              <div className={cn("flex items-center gap-2 sm:gap-3 overflow-hidden", !player.connected && "opacity-50 grayscale")}>
                <span className="text-xl flex-shrink-0 bg-slate-50 w-10 h-10 sm:w-11 sm:h-11 flex items-center justify-center rounded-xl border border-slate-100">
                  {player.icon ?? "🐶"}
                </span>
                
                <div className="flex flex-col min-w-0">
                  <span className="font-bold text-slate-800 text-sm sm:text-base truncate flex items-center gap-1.5">
                    {player.name}
                    {player.isHost && <Crown className="w-4 h-4 text-amber-500 fill-amber-500" />}
                  </span>
                  
                  {(player.streak ?? 0) > 1 && (
                    <span className="inline-flex items-center gap-1 text-xs font-bold text-orange-600 bg-orange-100 px-2 py-0.5 rounded-md w-fit">
            ...
            <div className="flex items-center gap-4 flex-shrink-0">
              {showScores && (
                <div className="flex flex-col items-end">
                  ...
                  <strong className="text-lg font-black text-slate-800">{player.score}</strong>
                </div>
              )}
