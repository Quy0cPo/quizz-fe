    ...
    <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar bg-slate-50/50">
      {messages.length === 0 && (
        <div className="h-full flex items-center justify-center">
          <p className="text-slate-400 font-medium bg-slate-100 px-4 py-2 rounded-xl">No messages yet. Say hi!</p>
        </div>
      )}
      ...
      {messages.map((msg, idx) => {
        ...
        return (
          ...
          >
            <span className="text-xs font-bold text-slate-500 mb-1 ml-1 flex items-center gap-1">
              {msg.icon || "👤"} {msg.name}
            </span>
            <div className={cn(
